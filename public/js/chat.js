   ...
   $(".chat-toggle").on("click", function (e) {
       ...
       cloneChatBox(user_id, username, function () {
           ...
           if(!chatBox.hasClass("chat-opened")) {
               ...
               loadLatestMessages(chatBox, user_id,$('#order_id').val());
...
/*               $(".chat-area").animate({ scrollTop: $(".chat-area")[0].scrollHeight }, 1000);*/
   ...
   $(".btn-chat").on("click", function (e) {
       send($(this).attr('data-to-user'), $(".chat_input").val(),$('#offer_id').val(),$('#order_id').val());
   });
   ...
   $(".chat-area").on("scroll", function (e) {
       ...
       if(st < lastScrollTop) {
           ...
           fetchOldMessages($(this).parents(".chat-opened").find("#to_user_id").val(), $(this).find(".msg_container:first-child").attr("data-message-id"),$('#order_id').val());
       }
 ...
 * @param user_id
 */
function loadLatestMessages(container, user_id,order_id)
{
    ...
    $.ajax({
        ...
        data: {order_id:order_id,user_id: user_id, _token: $("meta[name='csrf-token']").attr("content")},
        method: "GET",
        dataType: "json",
        beforeSend: function () {
 ...
 * @param message
 */
function send(to_user, message,offer_id,order_id)
{
    ...
    $.ajax({
        url: base_url + "/send",
        data: {order_id: order_id,offer_id: offer_id,to_user: to_user, message: message, _token: $("meta[name='csrf-token']").attr("content")},
        method: "POST",
        dataType: "json",
        beforeSend: function () {
        ...
        success: function (response) {
          ...
          $('.load_latest_offer').attr('data-id',response.data['message_id']);
           /*$(".chat-area").animate({ scrollTop: $(".chat-area")[0].scrollHeight }, 1000);*/
 ...
 */
function fetchOldMessages(to_user, old_message_id,order_id)
{
    ...
    $.ajax({
        ...
        data: {order_id:order_id,to_user: to_user, old_message_id: old_message_id, _token: $("meta[name='csrf-token']").attr("content")},
        method: "GET",
        dataType: "json",
        beforeSend: function () {
            if(chat_area.find(".loader").length  == 0) {
...
function getMessageSenderHtml(message)
{
  ...
  return `<p class="pt-3 pb-3 text-right">${message.content}</p>`;
/*    return `
...
function getMessageReceiverHtml(message)
{
  ...
  return `<p class="pt-3 pb-3">${message.content}</p>`;  
/*    return `
    ...
    if($("#current_user").val() == message.from_user_id) {
        ...
        cloneChatBox(message.from_user_id, message.fromUserName, function () {
            ...
            if(!chatBox.hasClass("chat-opened")) {
                ...
                loadLatestMessages(chatBox, message.from_user_id,$('#order_id').val());

                // $(".chat-area").animate({scrollTop: chatBox.find(".chat-area").offset().top + chatBox.find(".chat-area").outerHeight(true)}, 800, 'swing');
            } else {
